...
import { doc, collection, query, orderBy, limit, runTransaction, increment, where, getDocs, addDoc } from 'firebase/firestore';
...
export default function Dashboard() {
  ...
  const handleDepositRequest = async (type: 'bank' | 'crypto') => {
    if (!user || !depositAmount || !db || !profile) return;
    ...
    try {
      await addDoc(collection(db, 'deposits'), {
        ...
        status: 'pending',
        date: new Date().toISOString(),
        details: type === 'bank' ? { info: "Wire transfer pending" } : { info: "Crypto wallet pending" }
      });
    ...
    <div className="min-h-screen bg-background text-foreground font-body pb-32 relative overflow-hidden" onClick={() => setIsProfileOpen(false)}>
          ...
          <div className="bg-black/40 p-8 rounded-[3rem] shadow-[0_0_50px_rgba(212,175,55,0.1)] border border-primary/20 relative text-center max-w-[90%] w-[380px] flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative group mb-8">
              ...
              <div className="relative bg-white p-4 rounded-[2rem] border-4 border-primary/30 shadow-[0_0_30px_rgba(212,175,55,0.2)]">
                {qrCodeUrl ? <Image src={qrCodeUrl} alt="QR" width={208} height={208} className="w-52 h-52" /> : <div className="w-52 h-52 bg-muted flex items-center justify-center rounded-lg"><Loader2 className="h-10 w-10 text-primary animate-spin" /></div>}
              </div>
            </div>
            <div className="bg-primary/5 py-4 px-4 rounded-2xl border border-primary/20 flex flex-col items-center gap-1 cursor-pointer hover:bg-primary/10 transition-all w-full group overflow-hidden" onClick={() => { if(profile?.customId) { navigator.clipboard.writeText(profile.customId); toast({ title: t.idCopied }); }}}>
              ...
              <div className="flex items-center justify-center gap-2 w-full max-w-full px-2">
                <span className="font-headline font-black tracking-[0.05em] text-lg text-primary whitespace-nowrap overflow-hidden text-ellipsis">{profile?.customId || '---'}</span>
                <Copy size={16} className="text-primary/40 group-hover:text-primary transition-colors shrink-0" />
              </div>
            </div>
            <button onClick={() => setIsQrOpen(false)} className="mt-12 bg-white/5 text-white/40 hover:text-white p-4 rounded-full border border-white/10 hover:border-primary/50 transition-all"><X size={24} /></button>
          </div>
        </div>
      )}
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" placeholder={t.recipientPlaceholder} value={recipient} onChange={(e) => setRecipient(e.target.value.toUpperCase())} className="w-full bg-muted/50 border border-border rounded-xl py-3.5 px-4 text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:border-primary/50 text-center font-headline tracking-widest uppercase" />
                </div>
                <div className={cn("p-4 rounded-2xl border transition-all duration-300 flex items-center gap-4", isVerifying ? "bg-muted/20 border-border" : recipientName === 'SELF' ? "bg-red-500/5 border-red-500/20" : recipientName ? "bg-primary/5 border-primary/20" : "bg-muted/10 border-border")}>
                   <div className="w-10 h-10 rounded-full bg-background flex items-center justify-center border border-border">{isVerifying ? <Loader2 size={18} className="animate-spin text-primary" /> : <UserCheck size={18} className={recipientName ? "text-primary" : "text-muted-foreground/20"} />}</div>
                   <div className="flex-1 overflow-hidden"><p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest mb-0.5">{isVerifying ? t.verifying : recipientName ? t.userFound : t.userNotFound}</p><p className={cn("text-xs font-headline font-bold uppercase truncate", recipientName ? "text-foreground" : "text-muted-foreground/30")}>{recipientName === 'SELF' ? t.selfTransfer : (recipientName || '---')}</p></div>
                </div>
              </div>
              <div><label className="block text-xs text-muted-foreground mb-2 font-bold uppercase tracking-widest">{t.amountLabel}</label><input type="number" placeholder="0.00" value={sendAmount} onChange={(e) => setSendAmount(e.target.value)} className="w-full bg-muted/50 border border-border rounded-xl py-5 text-center text-4xl font-headline font-black text-primary placeholder:text-primary/10 focus:outline-none focus:border-primary/50" /></div>
            </div>
            <div className="p-6 pt-0"><button onClick={handleSendMoney} disabled={isSending || !recipientUid || !sendAmount || recipientName === 'SELF'} className="w-full bg-primary hover:bg-primary/90 disabled:opacity-50 text-primary-foreground font-headline font-black py-4 rounded-xl transition-all flex items-center justify-center gap-2"><span>{isSending ? t.sending : t.confirmSend}</span><ArrowUpRight size={20} /></button></div>
          </div>
        </div>
      )}
      ...
      {isDepositModalOpen && (
        <div className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-lg bg-card border-t sm:border border-border sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 duration-300">
            <div className="flex justify-between items-center p-6 border-b border-border bg-muted/30">
              <h2 className="text-xl font-headline font-bold text-foreground flex items-center gap-2"><PlusCircle className="text-secondary" />{t.depositHeader}</h2>
              <button onClick={() => setIsDepositModalOpen(false)} className="p-2 hover:bg-muted rounded-full transition-colors"><X size={20} className="text-muted-foreground" /></button>
            </div>
            <div className="p-6 space-y-6">
              <div>
                <label className="block text-xs text-muted-foreground mb-2 font-bold uppercase tracking-widest">{t.amountLabel}</label>
                <input type="number" placeholder="0.00" value={depositAmount} onChange={(e) => setDepositAmount(e.target.value)} className="w-full bg-muted/50 border border-border rounded-xl py-5 text-center text-4xl font-headline font-black text-secondary placeholder:text-secondary/10 focus:outline-none focus:border-secondary/50" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => handleDepositRequest('bank')} disabled={isDepositing || !depositAmount} className="flex flex-col items-center gap-3 p-4 bg-muted/30 border border-border rounded-2xl hover:border-primary transition-all">
                   <Building2 className="text-primary" size={24} />
                   <span className="text-[10px] font-headline font-bold uppercase tracking-widest">Bank Wire</span>
                </button>
                <button onClick={() => handleDepositRequest('crypto')} disabled={isDepositing || !depositAmount} className="flex flex-col items-center gap-3 p-4 bg-muted/30 border border-border rounded-2xl hover:border-secondary transition-all">
                   <Bitcoin className="text-secondary" size={24} />
                   <span className="text-[10px] font-headline font-bold uppercase tracking-widest">Crypto</span>
                </button>
              </div>
            </div>
          </div>
      ...
      <header className="flex justify-between items-center p-6 pt-8 relative z-[60]">
          ...
          {isProfileOpen && (
            <div onClick={(e) => e.stopPropagation()} className={cn("absolute top-14 w-64 bg-card/95 backdrop-blur-xl border border-border rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 z-[70]", language === 'ar' ? 'right-0' : 'left-0')}>
              <div className="p-4 border-b border-border bg-muted/30">
                ...
                <button onClick={() => { setIsQrOpen(true); setIsProfileOpen(false); }} className="w-full flex items-center justify-center gap-2 bg-primary/10 hover:bg-primary/20 p-2 rounded-lg border border-primary/20 transition-all group"><span className="text-[9px] text-primary font-headline font-bold uppercase tracking-widest">{t.showQr}</span><QrCode size={14} className="text-primary" /></button>
              </div>
              <div className="p-2 space-y-1">
                ...
                <div className="w-full flex flex-col gap-2 p-3 rounded-xl hover:bg-muted transition-all"><div className="flex items-center gap-3 mb-2"><Languages size={16} className="text-primary" /><span className="text-[11px] font-bold uppercase tracking-widest text-foreground/80">{t.switchLanguage}</span></div><div className="flex justify-center"><LanguageToggle /></div></div>
                <div className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-muted transition-all"><div className="flex items-center gap-3"><Moon size={16} className="text-primary" /><span className="text-[11px] font-bold uppercase tracking-widest text-foreground/80">{t.switchTheme}</span></div><ThemeToggle /></div>
        ...
        <div className="flex items-center gap-4"><div className="relative"><Bell size={24} className="text-foreground/80" /><span className="absolute top-0 right-0 w-2.5 h-2.5 bg-primary rounded-full border-2 border-background"></span></div></div>
      </header>
      ...
      <section className="px-6 mb-8 relative z-10 text-center">
        <div className="relative w-full p-8 rounded-[2rem] border border-border bg-card/40 backdrop-blur-2xl shadow-xl overflow-hidden group">
          ...
          <h1 className="text-5xl font-headline font-black text-foreground mb-4 tracking-tighter drop-shadow-2xl relative z-10">{profileLoading ? '...' : `$${profile?.balance?.toLocaleString() || '0'}`}<span className="text-2xl text-muted-foreground/20">.00</span></h1>
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-secondary/10 border border-secondary/20 relative z-10"><span className="w-2 h-2 rounded-full bg-secondary animate-pulse"></span><span className="text-[9px] text-secondary tracking-[0.2em] font-black uppercase">{t.secured}</span></div>
        </div>
      </section>
      ...
      <section className="px-6 rounded-t-[3rem] bg-muted/30 border-t border-border min-h-[400px] backdrop-blur-md pt-10 relative z-10">
        <div className="flex justify-between items-center mb-8 px-2"><h3 className="text-sm font-headline font-black uppercase tracking-widest text-foreground">{t.recent}</h3><button className="text-[10px] font-headline font-bold text-primary uppercase tracking-widest hover:underline">{t.seeAll}</button></div>
        <div className="space-y-4">
          {transactions.length === 0 ? <div className="text-center py-20 bg-card/40 rounded-[2rem] border border-dashed border-border"><p className="text-muted-foreground/30 text-[10px] font-headline font-bold uppercase tracking-widest">{t.noActivity}</p></div> : transactions.map((tx: any) => (
