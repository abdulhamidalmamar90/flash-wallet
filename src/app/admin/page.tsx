...
import { useStore, WithdrawalRequest } from '@/app/lib/store';
...
export default function AdminPage() {
  ...
  const { withdrawals, adminApprove, adminReject } = store;
  ...
  return (
    <div className="max-w-lg mx-auto p-6 space-y-8 animate-in fade-in slide-in-from-top-4 duration-700">
      <header className="flex justify-between items-center p-4 glass-card rounded-2xl border-primary/20 gold-glow">
        ...
        <Badge variant="outline" className="text-[8px] tracking-widest font-black uppercase text-primary border-primary/30">
          Superuser Access
        </Badge>
      </header>
                ...
                {req.status === 'pending' && (
                  <div className="flex gap-3 pt-2">
                    <button 
                      ...
                      className="flex-1 h-10 bg-primary/10 border border-primary/30 rounded-xl flex items-center justify-center gap-2 hover:bg-primary hover:text-background transition-all"
                    >
                      <Check className="h-4 w-4" />
                      <span className="text-[9px] font-headline font-bold tracking-widest">APPROVE</span>
                    </button>
                    <button 
                      onClick={() => handleReject(req.id)}
                      className="flex-1 h-10 bg-red-500/5 border border-red-500/20 rounded-xl flex items-center justify-center gap-2 hover:bg-red-500 hover:text-white transition-all"
                    >
                      <X className="h-4 w-4" />
                      <span className="text-[9px] font-headline font-bold tracking-widest">REJECT</span>
                    </button>
                  </div>
                )}
              </div>
