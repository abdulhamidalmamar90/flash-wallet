...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 px-6 pb-8 pt-2 pointer-events-none">
      <div className="mx-auto max-w-lg rounded-[2rem] flex items-center justify-around py-3 px-2 border bg-card/80 backdrop-blur-xl shadow-xl border-border pointer-events-auto transition-all duration-300">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={cn("h-5 w-5", isActive && "gold-glow")} />
